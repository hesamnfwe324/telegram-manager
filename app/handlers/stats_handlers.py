...
async def cb_stats(callback: CallbackQuery) -> None:
    ...
    text = (
        ...
        f"  ✅ امروز: <code>{s.today_joins}</code> گروه عضو شد\n\n"
        "🔗 <b>لینک‌ها</b>\n"
        f"  کل: <code>{s.total_links}</code> | در انتظار: <code>{s.pending_links}</code>\n\n"
        "👤 <b>کاربران مخاطب</b>\n"
        f"  ✅ فعال (قابل پیام): <code>{s.total_contacted_users}</code>\n"
        f"  📦 کل (شامل بلاک/غیرفعال): <code>{s.total_contacted_users_all}</code>\n\n"
        "⚙️ <b>سیستم</b>\n"
        f"  {health_icon} User Client: {'سالم' if s.client_healthy else 'مشکل دارد'}\n"
        f"  📋 صف عضویت: <code>{s.pending_queue_size}</code> مورد در انتظار\n"
